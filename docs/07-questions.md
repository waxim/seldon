...
questionVersion, engineVersion, seed)` — same tuple, same outcome.
...
shows a live matched-population count while the frame is edited. Because
frames use registry fields only, frame membership is uniform within a
[cell](02-lexicon.md), which is what keeps population-scale asking cheap
([engine](08-engine.md)). Honest limit, stated where relevant: the v1
replica does not model citizenship, so `registered` is the electoral
frame's boundary, and the outcome's provenance says so.
...
| `geo-rollup` | `level: L1..L4` | shares aggregated at seat / ward / area level for maps |
...
1. **Trigger.** A Second Foundation cron fires on cadence (and after
   every `polling_now` refresh). It asks Vault to create a run pairing
   `q-ge-vote-intent@4` with the newest generated `current-polling`
   scenario ([06](06-scenarios.md)) and the canon's latest epoch.
2. **Record.** Vault writes the run with its full reproducibility tuple,
   e.g. `(uk, epoch-2026-08-a, 3f9c…, q-ge-vote-intent@4, engine@1.4.0,
   20260824)`.
...
5. **Store.** The outcome lands in the Vault — D1 headline plus R2
   ...
   ```jsonc
   {
     ...
     "provenance": { "scenario": "current-polling@2026-08-24T06:00Z",
                     "epoch": "epoch-2026-08-a", "dataVersion": "…" }
   }
   ```
